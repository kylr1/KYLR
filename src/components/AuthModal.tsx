        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h3 className={`text-lg font-black ${isLight ? 'text-slate-900' : 'text-white'}`}>
              {isUnlocked ? 'وضع المالك مفعّل' : 'تسجيل دخول المالك'}
            </h3>
            <p className={`text-xs ${isLight ? 'text-slate-500' : 'text-slate-400'}`}>
              {isUnlocked
                ? 'هذا الجهاز مصرح له بتعديل كل القوائم والمعلومات.'
                : 'هذه النافذة مخصصة لمالك الموقع فقط لإدارة وتعديل المحتوى.'}
            </p>
          </div>
        </div>
            ...
            <div>
              ...
              <input
                ...
                className={`w-full px-4 py-3 rounded-xl text-sm border font-mono tracking-widest text-center focus:border-blue-500 focus:outline-none ${
                  isLight
                    ? 'bg-blue-50/40 border-blue-200 text-slate-900 placeholder:text-slate-400'
                    : 'bg-[#0f0f0f] border-blue-950 text-white placeholder:text-slate-600'
                }`}
              />
            </div>
